...
2. クラウド環境の許可ドメインに追加する（下記手順）
3. PDFを手動で `past_exams/` に置き、`python3 scripts/extract_exams.py` だけ実行する

抽出処理そのものは疑似PDFによるテストで検証済みです（後述）。

### クラウド環境に許可ドメインを追加する手順

1. [claude.ai/code](https://claude.ai/code) を開く
2. メッセージ入力欄の1つ上の行にある、環境名が表示された**雲アイコン**を選択
   （専用の設定ページやURLは無く、ここからしか開けません）
3. 対象の環境にカーソルを合わせ、右に出る**歯車アイコン**を選択
4. **Network access** を `Trusted` から **`Custom`** に変更
5. **Allowed domains** に1行1ドメインで追加:
...
6. **Also include default list of common package managers** に**チェックを入れる**
   （外すと PyPI に届かなくなり `pip install` が失敗します）
7. 保存する

> **設定は新しいセッションから有効になります。** 実行中のセッションは起動時の
> ネットワークポリシーを保持するため、保存後に**新しいセッションを開始**してください。
